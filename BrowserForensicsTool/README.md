...
3. **AI Intent Analysis**: Ensure `llama-cpp-python` and `huggingface-hub` are installed. Run with `--intent` to automatically download the 1.5B Qwen model (~1.1GB) to `~/.cache/browser_forensics_models`.
4. **Launch**: Execute `python main_gui.py` for the GUI or `python main.py --all --intent` for CLI collection.
